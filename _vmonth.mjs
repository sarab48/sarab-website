...
try {
  ...
  results.chipShown = (await chipA.count()) === 1
    && (await chipA.textContent()).includes('19:00')

  // Two bookings on one day, one of them confirmed → the hard clash flag.
  ...
  // The agenda under the grid lists the same day, in time order.
  ...
  // «المؤكدة فقط» drops the استفسار, keeps the real bookings.
  await page.locator('#cvRealOnly').check()
  await page.waitForTimeout(200)
  results.realOnlyFilters = (await page.locator('.cvchip', { hasText: 'زبون التقويم ج' }).count()) === 0
  ...
  const bar = await phone.locator('.cvchip', { hasText: 'زبون التقويم أ' }).boundingBox()
  const gridBox = await phone.locator('.cvgrid').boundingBox()
  results.phoneBars = !!bar && bar.height <= 9 && bar.height >= 4
